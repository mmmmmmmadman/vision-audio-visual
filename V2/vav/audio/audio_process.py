...
def audio_process_worker(
    ...
    # V2: Audio passthrough configuration
    passthrough_enabled = False
    passthrough_input_ch = 0
    passthrough_output_ch = 2
    ...
    def audio_callback(indata: np.ndarray, frames: int) -> np.ndarray:
        ...
        nonlocal passthrough_enabled, passthrough_input_ch, passthrough_output_ch, passthrough_gain
        ...
        try:
            while not control_queue.empty():
                ...
                # V2: Audio passthrough control
                elif msg_type == 'set_passthrough':
                    passthrough_enabled = msg.get('enabled', False)
                    passthrough_input_ch = msg.get('input_channel', 0)
                    passthrough_output_ch = msg.get('output_channel', 2)
                    passthrough_gain = msg.get('gain', 1.0)
                    print(f"[Audio] Passthrough: enabled={passthrough_enabled}, in={passthrough_input_ch}, out={passthrough_output_ch}, gain={passthrough_gain}")
        ...
        # Seq1 controls Alien4 Scan and Len (slice length)
        # Both use same 0.0-1.0 value from Seq1 CV
        alien4.set_scan(seq1_value)
        alien4.set_gate_threshold(seq1_value)  # Controls slice length via gateThresholdKnob
        ...
        # V2: Audio passthrough - route input directly to specified output
        if passthrough_enabled:
            if indata.shape[1] > passthrough_input_ch and audio_io.output_channels > passthrough_output_ch:
                outdata[:, passthrough_output_ch] = indata[:, passthrough_input_ch] * passthrough_gain
