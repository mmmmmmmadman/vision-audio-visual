...
from ..vision.media_cache import MediaCache
...
class VAVController:
    ...
    def __init__(self, config: dict = None):
        ...
        # Vision system
        self.camera: Optional[AsyncCamera] = None
        self.contour_cv_generator: Optional[ContourScanner] = None
        self.current_frame: Optional[np.ndarray] = None

        # V2: Media cache for folder-based region fill
        self.media_cache: Optional[MediaCache] = None
        self.use_media_cache = False
    ...
    def initialize(self) -> bool:
        ...
        print(f"[VAV] Camera initialized: {self.camera.width}x{self.camera.height}")
    ...
    def load_media_folder(self, folder_path: str) -> bool:
        """
        V2: Load media files from a folder into cache for region-based rendering.
        ...
        """
        try:
            # Create new MediaCache if not exists
            if self.media_cache is None:
                self.media_cache = MediaCache()

            # Load the folder
            success = self.media_cache.load_folder(folder_path)

            if success:
                self.use_media_cache = True
                print(f"[Controller] Media cache loaded: {self.media_cache.get_item_count()} items")
            else:
                print(f"[Controller] Failed to load media folder: {folder_path}")

            return success

        except Exception as e:
            import traceback
            print(f"[Controller] Error loading media folder: {e}")
            traceback.print_exc()
            return False

    def shuffle_media(self):
        """V2: Shuffle media selection for 4 regions"""
        if self.media_cache is not None and self.media_cache.is_loaded():
            self.media_cache.shuffle()
            print("[Controller] Media shuffled")
        else:
            print("[Controller] No media cache to shuffle")

    def get_media_count(self) -> int:
        """V2: Get number of loaded media items"""
        if self.media_cache is not None:
            return self.media_cache.get_item_count()
        return 0

    def set_passthrough(self, enabled: bool, input_channel: int = 0, output_channel: int = 2, gain: float = 1.0):
        """V2: Enable/disable audio passthrough"""
        if self.control_queue is not None:
            msg = {
                'type': 'set_passthrough',
                'enabled': enabled,
            ...
            try:
                ...
                print(f"[Controller] Passthrough: {'enabled' if enabled else 'disabled'} (in:{input_channel} → out:{output_channel})")
            except Exception as e:
                print(f"[Controller] Error setting passthrough: {e}")
    ...
    def _render_multiverse(self, frame, original_frame=None):
        ...
        # V2: Prepare media cache frames for region fill
        media_region_frames = None
        use_region_fusion = False

        if self.use_media_cache and self.media_cache is not None and self.media_cache.is_loaded():
            # Use media cache: get 4 frames for 4 regions
            target_size = (input_frame.shape[1], input_frame.shape[0])  # (width, height)
            media_region_frames = self.media_cache.get_region_frames(target_size)
            # Check if we have at least one valid frame
            if any(f is not None for f in media_region_frames):
        ...
        if use_gpu_region:
            ...
            rendered_rgb = self.renderer.render(
                ...
                compress=compress,
                use_region_fusion=use_region_fusion,
                media_region_frames=media_region_frames
            )
