...
class CompactMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.shuffle_btn.setFixedHeight(30)
        ...
        # V2: Passthrough toggle button
        self.passthrough_btn = QPushButton("Passthrough")
        self.passthrough_btn.setFixedHeight(30)
        self.passthrough_btn.setCheckable(True)
        self.passthrough_btn.clicked.connect(self._on_toggle_passthrough)
        ...
        control_layout.addWidget(self.passthrough_btn)
    ...
    def _on_select_devices(self):
        ...
        # V2: Get passthrough settings from stored current_devices
        stored_devices = getattr(self.controller, 'current_devices', {})
        ...
        current_devices = {
            ...
            'camera_output': None,
            'passthrough_input_ch': stored_devices.get('passthrough_input_ch', 0),
            'passthrough_output_ch': stored_devices.get('passthrough_output_ch', 2),
        }
        ...
        if devices:
            ...
            try:
                ...
                # V2: Store device settings for passthrough
                self.controller.current_devices = devices
    ...
    def _on_select_folder(self):
        """Select folder containing media files for region fill"""
        try:
            ...
            if folder_path:
                print(f"Loading media folder: {folder_path}")
                self.status_label.setText("Loading media folder...")

                # Use QTimer to defer loading to avoid blocking GUI thread
                def do_load():
                    success = self.controller.load_media_folder(folder_path)
                    if success:
                        count = self.controller.get_media_count()
                        self.status_label.setText(f"Loaded {count} media files")
                        print(f"Media folder loaded: {count} files")
                    else:
                        self.status_label.setText("Failed to load media folder")
                        print("Failed to load media folder")

                QTimer.singleShot(100, do_load)

        except Exception as e:
            import traceback
            print(f"Error in _on_select_folder: {e}")
            traceback.print_exc()
            self.status_label.setText(f"Error: {str(e)}")

    def _on_shuffle(self):
        """Shuffle media selection for regions"""
        try:
            self.controller.shuffle_media()
            self.status_label.setText("Media shuffled")
        except Exception as e:
            print(f"Error in _on_shuffle: {e}")
            self.status_label.setText(f"Error: {str(e)}")
    ...
    def _on_toggle_passthrough(self):
        """V2: Toggle audio passthrough"""
        try:
            enabled = self.passthrough_btn.isChecked()
            # Get passthrough settings from device dialog (stored in controller)
            devices = getattr(self.controller, 'current_devices', {})
            input_ch = devices.get('passthrough_input_ch', 0)
            output_ch = devices.get('passthrough_output_ch', 2)

            self.controller.set_passthrough(enabled, input_ch, output_ch)
            state_text = "ON" if enabled else "OFF"
            self.status_label.setText(f"Passthrough: {state_text} (in:{input_ch} → out:{output_ch})")
            print(f"[GUI] Passthrough {state_text}: input_ch={input_ch}, output_ch={output_ch}")

        except Exception as e:
            import traceback
            print(f"Error in _on_toggle_passthrough: {e}")
            traceback.print_exc()
            self.status_label.setText(f"Error: {str(e)}")
